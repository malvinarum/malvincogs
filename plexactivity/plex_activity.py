            ...
            async with self.session.get(image_url, timeout=5) as resp:
                if resp.status == 200:
                    ...
                    def get_color(img_data):
                        return Image.open(io.BytesIO(img_data)).convert("RGB").resize((1, 1)).getpixel((0, 0))
    ...
    async def _fetch_itunes_metadata(self, artist, title, album=None):
        ...
        try:
            async with self.session.get(search_url, params=params, timeout=5) as resp:
                if resp.status == 200:
                    # FIX: iTunes returns text/javascript, so we must tell aiohttp to ignore content_type
    ...
    async def _get_plex_sessions(self, guild_id: int):
        ...
        try:
            async with self.session.get(api_url, timeout=10) as response:
                ...
                sessions = []
                # Restored: search for Video, Photo, and Track tags specifically
                for session_elem in root.findall(".//Video") + root.findall(".//Photo") + root.findall(".//Track"):
                    ...
                    lib = session_elem.get("librarySectionTitle", "")
    ...
    async def plex_debugitunes(self, ctx, *, query: str):
        ...
        try:
            ...
            async with self.session.get("https://itunes.apple.com/search", params=params, timeout=10) as resp:
                # FIX: Added content_type=None to handle text/javascript mimetypes
                data = await resp.json(content_type=None)
                results = data.get("results", [])
